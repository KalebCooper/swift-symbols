...
package struct SymbolCatalog: Sendable {
  ...
  /// The newest SF Symbols year in the data, by numeric comparison.
  package let latestYear: String
  ...
  package static func build(from data: CoreGlyphsData) throws -> SymbolCatalog {
    ...
    for (old, current) in data.aliases.sorted(by: { $0.key < $1.key }) {
      ...
      // A localized pair follows the localized rule: the system picks the drawing itself.
      if SymbolName.isLocalized(current, canonical: canonical) { continue }
      if let dot = old.lastIndex(of: "."),
        SymbolName.localeTokens.contains(String(old[old.index(after: dot)...]))
      {
        continue
      }
    ...
    let latestYear =
      data.yearToRelease.keys.max { version($0).lexicographicallyPrecedes(version($1)) } ?? ""
